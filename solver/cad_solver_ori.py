...
class CADSolverOri(object):
    ...
    def create_srnn(self):
        ...
        # -------------------
        l_oreshapeback = lasagne.layers.ReshapeLayer(l_oconcat, (-1, shape3[1], shape3[2], 384))
        l_omerge = SumFeatureLayer(l_oreshapeback)

        # -------------------
        # Create the fusion layer
        # -------------------
        l_hconcat = llayers.ConcatLayer([l_lthh, l_omerge], axis=-1)
        ...
        l_lobj = llayers.LSTMLayer(l_oconcat, 256, grad_clipping=GRAD_CLIP)

        # -------------------
        # Create the output layer
        # -------------------
        l_sub_reshape = lasagne.layers.ReshapeLayer(l_lsub, (-1, 256))
        l_obj_reshape = lasagne.layers.ReshapeLayer(l_lobj, (-1, 256))

        self.l_actout = llayers.DenseLayer(l_lact, num_units=10, nonlinearity=lasagne.nonlinearities.softmax)
